...
task_backup=$(mktemp -d "$task_root/backup-0.3.6-XXXXXXXX")
...
trap rollback EXIT
cp server/server.js server/users.js server/library.js server/apps.js "$task_root/server/"
...
test "$task_healthy" = 1
task_complete=1
printf 'Actualizado a 0.3.6. Respaldo: %s\n' "$task_backup"
